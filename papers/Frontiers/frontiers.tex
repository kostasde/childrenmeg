...
Explaining \em{what} a trained neural network has learned is an on-going area of research, and a particularly important one. For many of these methods to be significantly useful beyond their successes as classifier tools, understanding what they are doing is crucial. There are currently two directions of work that address this, one in which (optionally modified) selections from datasets are fed into a model and the sensitivity of specific layers or neurons are examined, and a second in which artificial data is developed from the structure of the model itself (with no reliance on datapoints in particular). We take the latter approach along the lines of \cite{Yosinski2015} to develop artificial inputs that \em{maximally activate} key points in our models. This involves performing regularized gradient-ascent using the fully trained models.
