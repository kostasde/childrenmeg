...
The lateralization of speech in adults is


Acoustic changes in children's speech
Current thoughts on speech articulation


We approach this question by investigating a number of machine learning (ML) classifiers to predict the age of children performing both speech-elicitation tasks and verb-generation tasks.  MEG and EEG sensors, comes a denser stream of data that is unweildy for individual scientists to inspect and manage on their own. Although we apply techniques that summarize and digest our data into visually interpretable forms, this visually focused intake of data could benefit from examination with a new perspective. Machine learning (ML) in the form of trained regressors and classifiers are common throughout most data-heavy literature.
...
% Removed as this no longer plays a large enough factor in anything but comparison in the secondary dataset.
...
\begin{figure}[t]
  ...
  \caption{Graphical illustrations (using \cite{Delorme04eeglab}) of the independent components with features most correlated with audio features, from left to right, /{\em pah}/, /{\em pah tah kah}/ and verb generation experiments. These are generated using the ICA weight matrix interpolated over MEG sensor locations. The component depicted in A hints at significant activity localized in what could be called  Broca's region, and the component in C is more localized around Wernicke's region. These localizations seem appropriate according to classical interpretations of function for the tasks they are performing. Component B on the other hand is not so suggestive of classical language regions.}
