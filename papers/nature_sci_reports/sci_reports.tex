...
Code to use the SCNN and Ra-SCNN models is available at {\url https://github.com/SPOClab-ca/SCNN}. The models are implemented with Keras (\url{https://keras.io/}) with the intent that they can be dropped in as a black-box by an audience with some familiarity with Python and some relevant dataset. Necessary previous experience with neural networks is minimal. We have also included code to develop maximal activation components, and activation spectrograms for a trained SCNN model. For access to the data, please contact the corresponding author.
