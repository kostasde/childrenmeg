...
\newcommand{\heavymod}[1]{{\sethlcolor{orange}\hl{#1}}}
...
An advantage of this approach is that expert knowledge about the data can help circumvent relatively poor signal quality and encourage ML models to distinguish between classes using established correlates. However, this comes with potential bias of expert knowledge, and the underlying assumptions it may reinforce. \heavymod{In contrast, `deep' ML models can achieve many of the stages of the pipeline intrinsically, and consider potentially unknown correlates. We propose two new deep neural network (DNN) architectures, the first labelled SCNN is a mainly convolutional neural network, and the second labelled Ra-SCNN is an augmented SCNN network that employs a recurrent layer and attention mechanism. They are both outlined in figure} \ref{fig:archs}, \heavymod{and detailed below. Both networks are trained with raw single trials, that exploit the pipeline typically used for BCIs. Here they are trained to predict the age of normally developing (in terms of speech abilities) children aged 4-18, performing at least two of three tasks commonly used to evaluate speech and language development. They are evaluated with single trials of subjects unseen during training. This training and evaluation context is established to encourage the DNNs to make predictions on the grounds of detected differences in the development of speech and/or language.}

\heavymod{We compare the performance of a wide-range of classifiers, including our proposed models, when trained with features extracted from the MEG recordings, and as reference the audio recordings, of each trial. We herein refer to this process as a \textit{feature-based} approach. This is compared to our proposed DNNs trained with the same MEG recordings that have been minimally processed, which we call an \textit{end-to-end} approach. The best performing \textit{end-to-end} model is then thoroughly interrogated: we evaluate the learned weights' carry-over performance predicting which of the three speech tasks is being performed, we use activation maximization to examine the most successful network's inner workings, SOMETHING HERE ABOUT REST PREDICTION AND OBSCURING THINGS?} \st{and begin a discussion about the appropriate application of this technique to encephalographic data} \heavymod{and finally, we demonstrate that our models are a suitable classifier for encephalographic data more generally, by comparing the performance of our DNN architectures against the state of the art for a public dataset of EEG recordings used in BCI applications.}
...
\begin{table}[htp]
  \caption{Classification accuracy of $age >= 10$ versus $age < 10$. We compare standard feature classifiers of logistic regression, linear kernel support vector machine (Linear SVM), and a fully-connected feed-forward neural network (FFNN) to our proposed models trained end-to-end.}
  \centering
...
\newadd{Both of these networks comfortably exceed the level of chance, and the $38.13\%$ majority class threshold (as there is an imbalance of classes, which is addressed during training with a penalized loss). These accuracies are not as impressive as the age prediction task, but they do have performance that is remarkably similar to each other. A Wilcoxon signed-rank test shows these distributions to be similar with $W_{9, two-sided}=9.0$ and $p=0.8125$. Therefore, the weights (and thus features) learned to accurately perform the age classification task are equally usable to predict a separate speech task independent of age. We see this as evidence that the features are in some way speech related, rather than expressing characteristics correlated to age, and not speech.}

\subsection*{Model Activations}

\newadd{Maximizing the input at some point in the network with respect to an output somewhere downstream the network (both intermediate nodes and final outputs) can provide insight into the preferred characteristics of incoming data. This has been used previously to show the characteristics of individual layers, and preferred input for image classification networks} \cite{Yosinski2015}. We employed artificial input maximization using the model with the highest single fold test accuracy to minimize creating artificial data that had characteristics that were not pertinent to classifying age. This resulted in using the SCNN model trained with the second fold of the raw MEG dataset. Figure \ref{fig:max_components} demonstrates channel inputs (\newadd{bilinear interpolation over channel locations across scalp image}) that maximize three spatial filters at the final spatial summary layer, with respect to the input of the spatial summary. Correspondingly, figure \ref{fig:max_spectrograms} shows the spectral content of maximizing these same spatial features with respect to the two output classes, juxtaposed with high confidence real activations of again the same spatial features.
...
\newadd{The homogeneity of the artificial signals above could be evidence that classification is relying on some non-event related signals, for example some correlation between resting frequency and age} \cite{} \newadd{. To consider if this is the case, we again explore the high-accuracy SCNN model from above. We consider the predictions of at-rest recordings for each test subject and each experiment task. This is compared to the performance of subtracting the corresponding rest recording from an accurate test trial for each subject and test condition.}

\begin{table}[h]
 \caption{Number of correct predictions when considering at-rest recordings, collected for each subject and the tests they participated in. Compared to subtracting these rest data from highly confident and accurate test trials with corresponding subject and task.}
 \centering
 \begin{tabular}{l | c | c | c}
   \toprule
   \textbf{Predictions} & \textbf{Test Trials} & \textbf{Initial Rest} & \textbf{Test Minus Rest} \\
   \toprule
                        Correct           & 2686 & 15 & 18  \\
                        Incorrect         &  76  & 11 &  8  \\ 
   \bottomrule
 \end{tabular}
 \label{tab:rest_performance}
\end{table}

Table \ref{tab:rest_performance} \newadd{immediately suggests that the performance of rest data alone is dramatically lower than that of the un-modified trials. Subtracting rest from previously correctly classified test points seems to also severely reduce performance, although this group appears to be slightly better performing. Testing for independence of the \textit{Initial Rest} and \textit{Test Minus Rest} groups does not show a significant difference between the groups with $\chi^2_{2}=0.7464$ and $p<0.39$.}

\subsection*{Obscuring Trials}

\newadd{To more distinctly see what sections of a trial have the most impact on performance, figure} \ref{fig:obsc_profile} \newadd{plots the performance of the highly accurate SCNN model as a function of how much of the trial is obscured by white noise. We plot the performance of two different techniques: when the obscured portion is centred around the event prompt, labelled \textit{Obscure Event}, and obscuring the ends of the trial, labelled \textit{Obscure Ends}. Obscuring the trial event immediately reduces the performance of the model and steadily underperforms obscuring the ends. For approximately 250ms, obscuring the ends of a trial has no effect on performance, indicating that this portion of the data is likely unnecessary. What is of particular interest besides the consistent difference in performance, is the \textit{steady} decrease in performance both models demonstrate. The lack of any particularly sharp and distinct drop within the first half second makes it unlikely that some age correlated event-related field (ERF) is solely responsible for the performance we demonstrate, as most ERFs (or event-related potential in the case of EEG) are typically shorter than a half-second} \cite{ElectricFieldsOfTheBrain} \newadd{. If something like this were the main contributor to performance, once the entire ERF was fully obscured the performance should have immediately dropped. It is of course still possible that an ERF plays a role in classification decisions, but we argue that it is unlikely that they are a major contributor.

Obscuring the event gives a fairly consistent decrease in performance for at least the first two seconds of obscured points. Least-squares linear regression of this segment finds a slope of $-0.147$ ($p<<10^{-10}$ and correlation $r=-0.986$). Likewise when obscuring the ends, there appears to be a similar trend in performance between approximately 1 second and 3 seconds. For this segment the slope using least-squares linear regression is $-0.173$ ($p<<10^{-10}$ and correlation $r=-0.987$). Testing for similarity between the two slopes finds that the slope of these lines are not in fact the same ($p<<10^{-10}$, $t_{1394, two-sided}=-17.97$), so the decrease in performance does not happen identically in these sections.}
  
The curves in figure \ref{fig:obsc_profile} \newadd{seem to reflect performance that is dependent on the entire speech segment of the trials. These 2 second line segments account for an appropriate time around the event needed to complete each task REMOVEME(Looking into how hard it would be to actually figure out mean+dev of speaking time, make this argument clearer).}
...
These results are interesting as a demonstration of the ability of end-to-end models to predict \st{language} \newadd{speech} development, but neural networks show perhaps even greater promise as computational modelling techniques that to some degree mimic the human brain. For example \cite{cichy2016} compare the activations of an image classifier CNN and the MEG and fMRI activity of 15 subjects viewing the same test objects as the CNN. \heavymod{The architectures we present here are very general-purpose, a trait they receive from the fact that they mimic the BCI pipeline, but future work should consider deep neural networks that mimic architectures that follow speech models with suggested neural substrates, such as the DIVA model}\cite{Guenther2005}. \heavymod{These could be used to predict speech output or articulator positions with data such as ours, and explore their connection to the hypothesized brain structures.}
...
To produce figure \ref{fig:obsc_profile} \newadd{we averaged \textit{profiles} created for each test subjects in the test dataset, for each of the highest confidence (defined here as largest correct class output) test points within each task the subject performed. Each \textit{profile} consists of 10 different uniform noise signals that grow from the event out to the ends in the case of \textit{Obscure Event} or from the ends towards the event in \textit{Obscure Ends}. When we say obscuring, this consists of a new point created out of the original signal alone for all points that are not obscured, and a weighted combination of the original signal and a noise signal, at a relative weighting of $1:1000$ respectively for the section that is obscured. This obscuring technique then \textit{grows} by the addition of a single sample to the set of obscured points (and the respective removal of that point from the un-obscured set). For each sample obscured and each of the 10 noise signals, these modified points are fed to the pre-trained model and the model output is stored. Since the event prompt is localized at $t=0.5s$ within a trial that is $3.5$ seconds long, the growing process proceeds at a rate of 6 samples to 1. Where 6 samples are added after the event at $t=0.5s$ for each added before in the case of \textit{Obscure Event}, and 6 samples are added to the end of the trial for every 1 added to the beginning in the case of \textit{Obscure Ends}.}
...
All hyperparameter selection is done using the \newadd{tree-structured Parzen estimator} implemented in Hyperopt \cite{Bergstra2013} including: learning rate, regularization penalty, dropout rates, number of adjacent activations to pool, number and layers of hidden units, receptive fields, and activation functions for all appropriate models. Hyperparameter searches are performed for 100 iterations.
