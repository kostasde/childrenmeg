...
An advantage of this approach is that expert knowledge about the data can help circumvent relatively poor signal quality and encourage ML models to distinguish between classes using established correlates. However, this comes with potential bias of expert knowledge, and the underlying assumptions it may reinforce. \heavymod{In contrast, `deep' ML models can achieve many of the stages of the pipeline intrinsically, and consider potentially unknown correlates. We propose new deep neural network (DNN) architectures trained end-to-end, with raw single trials, that exploit the pipeline typically used for BCIs. These DNNs are trained to predict the age of normally developing children aged 4-18, performing tasks used to evaluate the development of speech. They are evaluated with single trials of subjects previously unseen during their training that mimic the distribution of training data. This provides the DNNs a context that encourages predictions to be made on the grounds of detected differences in the development of speech. We compare the performance of a wide-range of classifiers, including our proposed models, when trained with pre-extracted features and compare these to the performance of our DNNs trained with raw samples that have been minimally processed. We use activation maximization to examine the most successful network's inner workings and begin a discussion about the appropriate application of this technique to encephalographic data. Finally to ensure that our models are a suitable classifier for encephalographic data, we benchmark the performance of our DNN architectures against the state of the art for a public dataset of EEG recordings used in BCI applications.}

NEED TO ADD SOMETHING STILL ABOUT WHAT THE AUDIO STUFF IS DOING THERE, IT LOOKS LIKE WE ARE SOLVING THE ENGINEERING PROBLEM OF AGE PREDICTION.
